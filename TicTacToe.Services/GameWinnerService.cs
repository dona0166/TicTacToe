...
namespace TicTacToe.Services
{
    ...
    public class GameWinnerService : IGameWinnerService
    {
        public char Validate(char[,] gameBoard)
        {
            ...
            var columnFourChar = gameBoard[1, 0];
            var columnFiveChar = gameBoard[1, 1];
            var columnSixChar = gameBoard[1, 2];
            if((columnOneChar == columnTwoChar && columnTwoChar == columnThreeChar))
            {
                return columnOneChar;
            }
            else if(columnFourChar == columnFiveChar && columnFiveChar == columnSixChar)
            {
                return columnFourChar;
            }
